...
SITUATION_ANALYZER_PROMPT = ChatPromptTemplate.from_messages([
    ...
    在分析时，必须严格遵守以下两点：
    1. 禁止将此类节点的内容归因于学生，禁止出现"学生想到了……"、"学生已经……"等表述。
    2. 此类节点只能说明"系统计划引导学生朝某方向尝试"，不代表学生有任何进展。
    ...
    【分析维度】
    1. 解题状态：学生本轮是否取得了真实进展？（基于对话历史中学生的实际表述，而非思维树中的【引导方向】节点）
