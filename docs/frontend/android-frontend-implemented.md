...
6. 已新增一块最小可用体验区，支持手动输入题目、创建 Guide 会话、发送一轮学生思路并接收老师引导追问。

当前实现特点：

1. 页面重点在于明确用户进入解题流程的入口。
2. solve 页面仍保留 `solveRouteMockUiState()` 作为视觉主骨架，但已叠加真实 Guide API 最小接线区。
3. Android 侧已新增 `GuideApiClient`，开始调用 Guide 后端创建会话与非流式轮次接口。
4. 追及问题、鸡兔同笼已经在页面文本层面突出，符合当前 demo 聚焦题型。

当前未实现内容：

1. 拍照、相册入口的真实功能。
2. OCR 识别。
3. 流式输出、报告生成、题解/可视化等 Guide 完整链路。
4. Guide 结果沉淀到 mistakes 的闭环。
5. 分步可视化讲解的完整交互与真机联调验收。
...
1. 目前已经存在两层实现：
	- 历史静态首页 UI
	- 联网版多页面错题流程（首页、时间轴、详情、重做、推荐、讲给 AI 听）
2. 主入口 `PracticeRoute` 已切换为联网版状态机容器，不再默认展示静态 mock 首页。
3. `MistakesApiClient` 已接入后端 mistakes 接口族。
4. 二级页面进入后会隐藏底部栏，交互形态已与 Preview/Profile 的多级页面保持一致。

当前未完全验收内容：

1. 还缺少真机 / 模拟器下的完整联调回归记录。
2. 固定 LAN 地址仍需进一步配置化，避免环境切换问题。
3. 文档与实现已基本接近，但仍需补齐运行截图或验收记录，才能将其视为“产品级可用”。
